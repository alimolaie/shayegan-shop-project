<?php
$footerMenusTrees   = App\Categories::CategoriesTree();
$footerAboutDetails = App\Http\Controllers\webController::singlePageDetails(4);
$privacy_details    = App\Http\Controllers\webController::singlePageDetails(2);
$terms_details      = App\Http\Controllers\webController::singlePageDetails(3);
$about_details      = App\Http\Controllers\webController::singlePageDetails(1);
?>
<?php if($settingInfo->social_whatsapp): ?>
<?php
if($settingInfo->theme==1){
$curtime     = strtotime(date('h:i a'));
$firsttime1  = strtotime('09:00 am');
$firsttime2  = strtotime('09:00 pm');
if($curtime>=$firsttime1 && $curtime < $firsttime2){
$whatsAppNumber = "96566615592";
}else{
$whatsAppNumber = "96594192932";
}
}else{
$whatsAppNumber = $settingInfo->social_whatsapp;
}

?>
<?php if(!empty($settingInfo->is_float_whatsapp)): ?>
<a href="https://api.whatsapp.com/send?phone=<?php echo e($whatsAppNumber); ?>&text=<?php echo e(__('webMessage.whatsappsharetext')); ?>" class="float" target="_blank"><img width="60" height="60" src="data:image/png;base64,"/></a>
<?php endif; ?>

<?php endif; ?> 
<footer class="f-mobile-dark">
	<div class="tt-footer-col tt-color-scheme-01">
		<div class="container">
			<div class="row">
				<div class="col-md-6 col-lg-2 col-xl-3">
					<div class="tt-mobile-collapse">
                       <?php if(!empty($footerMenusTrees) && count($footerMenusTrees)>0): ?>
						<h4 class="tt-collapse-title">
							<?php echo e(strtoupper(__('webMessage.categories'))); ?>

						</h4>
						<div class="tt-collapse-content">
							<ul class="tt-list">
                              <?php $__currentLoopData = $footerMenusTrees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $footerMenusTree): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
								<li><a href="<?php echo e(url('/products/'.$footerMenusTree->id.'/'.$footerMenusTree->friendly_url)); ?>"><?php if(app()->getLocale()=="en"): ?> <?php echo e($footerMenusTree->name_en); ?> <?php else: ?> <?php echo e($footerMenusTree->name_ar); ?> <?php endif; ?></a></li>
                               <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?> 
							</ul>
						</div>
                        <?php endif; ?>
					</div>
				</div>
				<div class="col-md-6 col-lg-2 col-xl-3">
					<div class="tt-mobile-collapse">
						<h4 class="tt-collapse-title">
							<?php echo e(strtoupper(__('webMessage.importantlinks'))); ?>

						</h4>
						<div class="tt-collapse-content">
							<ul class="tt-list">
							    <li><a href="<?php echo e(url('/page/'.$about_details->slug)); ?>"><?php echo e(__('webMessage.aboutus')); ?></a></li>
								<li><a href="<?php echo e(url('/contactus')); ?>"><?php echo e(__('webMessage.contactus')); ?></a></li>
								<?php if(!empty(Auth::guard('webs')->user()->id)): ?>
								<li><a href="<?php echo e(url('/dashboard')); ?>"><?php echo e(__('webMessage.myaccount')); ?></a></li>
								<?php else: ?>
								<li><a href="<?php echo e(url('/register')); ?>"><?php echo e(__('webMessage.signup')); ?></a></li>
								<li><a href="<?php echo e(url('/login')); ?>"><?php echo e(__('webMessage.signin')); ?></a></li>
								<?php endif; ?>
                                <li><a href="<?php echo e(url('/page/'.$privacy_details->slug)); ?>"><?php echo e(__('webMessage.privacypolicy')); ?></a></li>
                                <li><a href="<?php echo e(url('/page/'.$terms_details->slug)); ?>"><?php echo e(__('webMessage.termsconditions')); ?></a></li>
                                <li><a href="<?php echo e(url('/faq')); ?>"><?php echo e(__('webMessage.faq')); ?></a></li>
							</ul>
						</div>
					</div>
				</div>
				<div class="col-md-6 col-lg-4 col-xl-3">
					<div class="tt-mobile-collapse">
						<h4 class="tt-collapse-title">
							<?php echo e(strtoupper(__('webMessage.contactus'))); ?>

						</h4>
						<div class="tt-collapse-content">
							<address>
                                <?php if(app()->getLocale()=="ar" && $settingInfo->address_ar): ?>
								<p><span><?php echo e(__('webMessage.address')); ?>:</span> <?php echo e($settingInfo->address_ar); ?></p>
                                <?php endif; ?>
                                <?php if(app()->getLocale()=="en" && $settingInfo->address_en): ?>
								<p><span><?php echo e(__('webMessage.address')); ?>:</span> <?php echo e($settingInfo->address_en); ?></p>
                                <?php endif; ?>
                                <?php if($settingInfo->phone): ?>
								<p><span><?php echo e(__('webMessage.phone')); ?>:</span> <?php echo e($settingInfo->phone); ?></p>
                                <?php endif; ?>
                                <?php if(app()->getLocale()=="ar" && $settingInfo->office_hours_ar): ?>
                                <p><span><?php echo e(__('webMessage.officehours')); ?>:</span> <?php echo e($settingInfo->office_hours_ar); ?></p>
                                <?php elseif(app()->getLocale()=="en" && $settingInfo->office_hours_en): ?>
                                <p><span><?php echo e(__('webMessage.officehours')); ?>:</span> <?php echo e($settingInfo->office_hours_en); ?></p>
                                <?php endif; ?>
                                <?php if($settingInfo->email): ?>
								<p><span><?php echo e(__('webMessage.email')); ?>:</span> <a href="mailto:<?php echo e($settingInfo->email); ?>"><?php echo e($settingInfo->email); ?></a></p>
                                <?php endif; ?>
							</address>
						</div>
					</div>
				</div>
				<div class="col-md-6 col-lg-4 col-xl-3">
					<div class="tt-newsletter">
						<div class="tt-mobile-collapse">
							<h4 class="tt-collapse-title">
								<?php echo e(strtoupper(__('webMessage.newslettersignup'))); ?>

							</h4>
							<div class="tt-collapse-content">
                            <?php if(app()->getLocale()=="en" && $settingInfo->newsletter_note_en): ?>
							<p><?php echo $settingInfo->newsletter_note_en; ?></p>
                            <?php endif; ?>
                            <?php if(app()->getLocale()=="ar" && $settingInfo->newsletter_note_ar): ?>
							<p><?php echo $settingInfo->newsletter_note_ar; ?></p>
                            <?php endif; ?>
                            
                                
								<form id="newsletterformtxt" name="newsletterformtxt" class="form-inline form-default" method="post" novalidate="novalidate">
									<div class="form-group">
										<input type="text" name="newsletter_email" id="newsletter_email" class="form-control" placeholder="<?php echo e(__('webMessage.enter_email')); ?>"><span id="newslettermsg"></span>
                                        
										<button type="button" class="btn" id="subscribeBtn"><?php echo e(__('webMessage.subscribe')); ?></button>
									</div>
								</form>
							</div>
						</div>
					</div>
					<ul class="tt-social-icon">
                        <?php if($settingInfo->social_facebook): ?>
						<li><a title="<?php echo e(__('webMessage.facebook')); ?>" class="icon-g-64" target="_blank" href="<?php echo e($settingInfo->social_facebook); ?>"></a></li>
                        <?php endif; ?>
                        <?php if($settingInfo->social_twitter): ?>
						<li><a title="<?php echo e(__('webMessage.twitter')); ?>" class="icon-h-58" target="_blank" href="<?php echo e($settingInfo->social_twitter); ?>"></a></li>
                        <?php endif; ?>
                        <?php if($settingInfo->social_instagram): ?>
						<li><a title="<?php echo e(__('webMessage.instagram')); ?>" class="icon-g-67" target="_blank" href="<?php echo e($settingInfo->social_instagram); ?>"></a></li>                        <?php endif; ?>
                        <?php if($settingInfo->social_linkedin): ?>
						<li><a title="<?php echo e(__('webMessage.linkedin')); ?>" class="icon-g-68" target="_blank" href="<?php echo e($settingInfo->social_linkedin); ?>"></a></li>
                        <?php endif; ?>
                        <?php if($settingInfo->social_youtube): ?>
						<li><a title="<?php echo e(__('webMessage.youtube')); ?>" class="icon-g-76" target="_blank" href="<?php echo e($settingInfo->social_youtube); ?>"></a></li>
                        <?php endif; ?>
					</ul>
				</div>
			</div>
		</div>
	</div>
	<div class="tt-footer-custom tt-color-scheme-04">
		<div class="container">
			<div class="tt-row">
				<div class="tt-col-left">
					<div class="tt-col-item tt-logo-col">
                        <?php if($settingInfo->logo): ?>
						<a class="tt-logo tt-logo-alignment" href="<?php echo e(url('/')); ?>"><img src="<?php echo e(url('uploads/logo/'.$settingInfo->footerlogo)); ?>" title="<?php if(app()->getLocale()=="en"): ?> <?php echo e($settingInfo->name_en); ?> <?php else: ?> <?php echo e($settingInfo->name_ar); ?> <?php endif; ?>" alt="<?php if(app()->getLocale()=="en"): ?> <?php echo e($settingInfo->name_en); ?> <?php else: ?> <?php echo e($settingInfo->name_ar); ?> <?php endif; ?>"></a>
                        <?php endif; ?>
					</div>
					<div class="tt-col-item">
						<div class="tt-box-copyright">
							<?php if($settingInfo->copyrights_en && app()->getLocale()=="en"): ?><?php echo $settingInfo->copyrights_en; ?><?php endif; ?>
                            <?php if($settingInfo->copyrights_ar && app()->getLocale()=="ar"): ?><?php echo $settingInfo->copyrights_ar; ?><?php endif; ?>
						</div>
					</div>
				</div>
				<div class="tt-col-right">
					<div class="tt-col-item">
					    <?php if(!empty($settingInfo->payments)): ?>
                        <?php
                        $payments = explode(",",$settingInfo->payments);
                        ?>
						<ul class="tt-payment-list">
						    <?php $__currentLoopData = $payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<li><a href="javascript:;"><img src="<?php echo e(url('uploads/paymenticons/'.strtolower($payment).'.png')); ?>" height="30" alt=""></a></li>
							<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
						</ul>
						<?php endif; ?>
					</div>
				</div>
			</div>
		</div>
	</div>
</footer><?php /**PATH C:\Users\Lenovo\Downloads\well-known\resources\views/website/includes/footer_theme1.blade.php ENDPATH**/ ?>