...
<html lang="en">
	...
	<head>
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.editservice')}}</title>
			...
			<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--ver kt-page">
				...
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
					...
					<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.editservice')}}</a>
									</div>
								</div>
						...
						<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">{{__('adminMessage.editservices')}}</h3>
									</div>
                    ...
                    <form name="tFrm"  id="form_validation"  method="post"
                          class="kt-form" enctype="multipart/form-data" action="{{route('services.update',$editservices->id)}}">
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                           ...
                                           <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.menu_name_en')}}</label>
                                                        <input type="text" class="form-control @if($errors->has('menu_name_en')) is-invalid @endif" name="menu_name_en"
                                                               value="{{$editservices->menu_name_en?$editservices->menu_name_en:old('menu_name_en')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_menu_name_en')}}*" />
                                                ...
                                                <label>{{__('adminMessage.menu_name_ar')}}</label>
                                                        <input type="text" class="form-control @if($errors->has('menu_name_ar')) is-invalid @endif" name="menu_name_ar"
                                                               value="{{$editservices->menu_name_ar?$editservices->menu_name_ar:old('menu_name_ar')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_menu_name_ar')}}*" />
                                                ...
                                                <div class="form-group row">
													...
													<div class="col-3">
														<span class="kt-switch">
															<label>
																<input type="checkbox" {{$editservices->is_active==1?'checked':''}} name="is_active"  id="is_active" value="1"/>
																<span></span>
															</label>
														</span>
													</div>
													...
													<div class="col-3">
														<input type="text" class="form-control @if($errors->has('display_order')) is-invalid @endif" name="display_order"  value="{{$editservices->display_order?$editservices->display_order:old('display_order')}}" autocomplete="off" />
                                       ...
                                       <!--categories name -->         
                                                ...
                                                <input type="text" class="form-control @if($errors->has('title_en')) is-invalid @endif" name="title_en"
                                                               value="{{$editservices->title_en?$editservices->title_en:old('title_en')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_title_en')}}*" />
                                                ...
                                                <input type="text" class="form-control @if($errors->has('title_ar')) is-invalid @endif" name="title_ar"
                                                               value="{{$editservices->title_ar?$editservices->title_ar:old('title_ar')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_title_ar')}}*" />
                                      ...
                                      <!--categories description -->          
                                            <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.details_en')}}</label>
                                                        <textarea rows="3" id="details_en" name="details_en" class=" kt-tinymce-4 form-control @if($errors->has('details_en')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enter_details_en')}}">{!!$editservices->details_en?$editservices->details_en:old('details_en')!!}</textarea>
                                                ...
                                                <label>{{__('adminMessage.details_ar')}}</label>
                                                        <textarea   rows="3" id="details_ar" name="details_ar" class="kt-tinymce-4 form-control @if($errors->has('details_ar')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enter_details_ar')}}">{!!$editservices->details_ar?$editservices->details_ar:old('details_ar')!!}</textarea>
                                      ...
                                      <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.seokeywords_en')}}</label>
                                                        <textarea rows="3" name="seo_keywords_en" class="form-control @if($errors->has('seo_keywords_en')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enterseokeywords_en')}}">{{$editservices->seo_keywords_en?$editservices->seo_keywords_en:old('seo_keywords_en')}}</textarea>
                                                ...
                                                <label>{{__('adminMessage.seokeywords_ar')}}</label>
                                                        <textarea rows="3" name="seo_keywords_ar" class="form-control @if($errors->has('seo_keywords_ar')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enterseokeywords_ar')}}">{{$editservices->seo_keywords_ar?$editservices->seo_keywords_ar:old('seo_keywords_ar')}}</textarea>
                                    ...
                                    <div class="form-group row">
                                                ...
                                                <label>{{__('adminMessage.seodescription_en')}}</label>
                                                        <textarea rows="3" name="seo_description_en" class="form-control @if($errors->has('seo_description_en')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enterseodescription_ar')}}">{{$editservices->seo_description_en?$editservices->seo_description_en:old('seo_description_en')}}</textarea>
                                                ...
                                                <label>{{__('adminMessage.seodescription_ar')}}</label>
                                                        <textarea rows="3" name="seo_description_ar" class="form-control @if($errors->has('seo_description_ar')) is-invalid @endif" autocomplete="off" placeholder="{{__('adminMessage.enterseodescription_ar')}}">{{$editservices->seo_description_ar?$editservices->seo_description_ar:old('seo_description_ar')}}</textarea>
