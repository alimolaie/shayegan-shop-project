...
<html lang="en">
	...
	<head>
		...
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.newsevents')}}</title>
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.newseventslisting')}}</a>
								...
								<div class="kt-subheader__toolbar">
									...
									<div class="btn-group">
										...
										<a href="{{url('gwc/newsevents/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
                                        @endif
										...
										<div class="dropdown-menu dropdown-menu-right">
											<ul class="kt-nav">
                                                ...
                                                <li class="kt-nav__item">
													<a href="{{url('gwc/newsevents/pdf')}}" class="kt-nav__link">
							...
							<div class="kt-portlet kt-portlet--mobile">
								<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">
											{{__('adminMessage.newseventslisting')}}
										</h3>
									</div>
								</div>
                                ...
                                @if(auth()->guard('admin')->user()->can('newsevents-list'))
									...
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
                                        ...
                                        @php $p=1; @endphp
                                        @foreach($newseventsLists as $key=>$newseventsList)
											<tr class="search-body">
												<td>{{$newseventsLists->firstItem() + $key}}</td>
												<td>{!! $newseventsList->title_en !!}</td>
												<td>{!! $newseventsList->title_en !!}</td>
												<td>
                                                @if($newseventsList->image)
                                                <img src="{!! url('uploads/newsevents/thumb/'.$newseventsList->image) !!}" width="40">
                                                @endif</td>
												<td>
                                                <span class="kt-switch"><label><input value="{{$newseventsList->id}}" {{!empty($newseventsList->is_active)?'checked':''}} type="checkbox"  id="newsevents" class="change_status"><span></span></label></span>
                                                </td>
												<td>{!! $newseventsList->created_at !!}</td>
												<td>{!! $newseventsList->updated_at !!}</td>
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <li class="kt-nav__item"><a href="{{url('gwc/newsevents/'.$newseventsList->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">{{__('adminMessage.edit')}}</span></a></li>
                                                 @endif
                                                 @if(auth()->guard('admin')->user()->can('newsevents-delete'))
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$newseventsList->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 @endif
                                                 </ul>
                                                 </div>
                                                 </div>
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$newseventsList->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/newsevents/delete/'.$newseventsList->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
									</div>
								</div>
							</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="8" class="text-center">{{ $newseventsLists->links() }}</td></tr> 
                                        @else
