...
<html lang="en">
	...
	<head>
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.editcustomers')}}</title>
			...
			<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--ver kt-page">
				...
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
					...
					<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="{{url('gwc/customers')}}" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.customers')}}</a>
										<span class="kt-subheader__breadcrumbs-separator"></span>
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.editcustomers')}}</a>
									</div>
								</div>
                                <div class="kt-subheader__toolbar">
                                <a href="{{url('gwc/customers')}}" class="btn btn-default btn-bold">{{__('adminMessage.back')}}</a>
								</div>
						...
						<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">{{__('adminMessage.editcustomers')}}</h3>
									</div>
                    ...
                    <form name="tFrm"  id="form_validation"  method="post"
                          class="kt-form" enctype="multipart/form-data" action="{{route('customers.update',$editcustomers->id)}}">
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                           ...
                                           <div class="form-group"><h5>{{__('adminMessage.basicinformation')}}</h5></div>
                                            <div class="form-group row">
                                                ...
                                                <input type="text" class="form-control @if($errors->has('name')) is-invalid @endif" name="name"
                                                               value="{{$editcustomers->name?$editcustomers->name:old('name')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_name')}}*" />
                                                               @if($errors->has('name'))
                                                ...
                                                <input type="text" class="form-control @if($errors->has('email')) is-invalid @endif" name="email"
                                                               value="{{$editcustomers->email?$editcustomers->email:old('email')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_email')}}*" />
                                                               @if($errors->has('email'))
                                         ...
                                         <div class="form-group row">
                                                ...
                                                <input type="text" class="form-control @if($errors->has('mobile')) is-invalid @endif" name="mobile"
                                                               value="{{$editcustomers->mobile?$editcustomers->mobile:old('mobile')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_mobile')}}*" />
                                                ...
                                                <div class="col-lg-2">
                                                @if($editcustomers->image)
                                                <img src="{!! url('uploads/customers/thumb/'.$editcustomers->image) !!}" width="40">
                                                <a href="{{url('gwc/customers/deletecustomersImage/'.$editcustomers->id)}}" class="btn btn-brand btn-danger btn-icon-sm btn-sm"><i class="la la-trash"></i>{{__('adminMessage.delete')}}</a>
                                                @endif
                                                </div>
                                            </div>
                                                ...
                                                <div class="form-group row">
													...
													<div class="col-3">
														<span class="kt-switch">
															<label>
																<input type="checkbox" {{$editcustomers->is_active==1?'checked':''}} name="is_active"  id="is_active" value="1"/>
																<span></span>
															</label>
														</span>
													</div>
