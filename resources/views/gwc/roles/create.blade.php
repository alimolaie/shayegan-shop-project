...
<html lang="en">
	...
	<head>
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}} | {{__('adminMessage.createnewrole')}}</title>
				...
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.createnewrole')}}</a>
									</div>
								</div>
					...
					@if(auth()->guard('admin')->user()->can('role-create'))
                         ...
                         <form name="tFrm"  id="form_validation"  method="post"
                          class="kt-form" enctype="multipart/form-data" action="{{route('roles.store')}}">
											...
											<div class="form-group row"><h6>{{__('adminMessage.rolename')}}</h6></div> 	
                                                ...
                                                <div class="col-lg-6">
                                                        <input type="text" class="form-control @if($errors->has('name')) is-invalid @endif" name="name"
                                                               value="{{old('name')}}" autocomplete="off" placeholder="{{__('adminMessage.enterrolename')}}" />
                                                               @if($errors->has('name'))
                                                 ...
                                                 <div class="kt-checkbox-list kt-checkbox-inline">
                                                    ...
                                                    @foreach($permission as $value)
                                                       <div class="col-lg-3">
                                                        ...
                                                        <input type="checkbox" name="permission[]" id="per-{{$value->id}}" value="{{$value->id}}">
                                                        {{ $value->name }}
                                                        <span></span>
                                                        </label>
                                                        </div>
