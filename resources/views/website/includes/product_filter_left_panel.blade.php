...
<div class="col-md-4 col-lg-3 col-xl-3 leftColumn aside desctop-no-sidebar ">
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							...
							<a href="javascript:;" id="clearallfilter" class="btn-link-02">{{strtoupper(__('webMessage.clearall'))}}</a>
						</div>
					</div>
                    @endif
                    <!--end filter history -->
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-row">
                                ...
                                @endphp
								<li><a href="{{url('products/'.$productCategory->id.'/'.$productCategory->friendly_url)}}">@if(app()->getLocale()=="en" && !empty($productCategory->name_en)) {{$productCategory->name_en}} @elseif(app()->getLocale()=="ar" && $productCategory->name_ar) {{$productCategory->name_ar}} @endif <span class="float-right">({{$countProductss}})</span></a></li>
                                @endforeach
							</ul>
						</div>
					</div>
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-row">
								...
								<li><a href="javascript:;" id="{{$i}}-{{$i+15}}" class="rangeprice"> {{$i}} {{__('webMessage.kd')}} —  {{$i+15}} {{__('webMessage.kd')}}</a></li>
                                @endfor
							</ul>
						</div>
					</div>
						...
						<div class="tt-collapse-content">
							<ul class="tt-options-swatch options-middle">
                               @foreach($prodSizes as $prodSize)
								<li @if(!empty(Cookie::get('filter_by_size')) && Cookie::get('filter_by_size')==$prodSize->id) class="active" @endif><a href="javascript:;" class="filter_by_size" id="{{$prodSize->id}}">@if(app()->getLocale()=="en" && !empty($prodSize->title_en)) {{$prodSize->title_en}} @elseif(app()->getLocale()=="ar" && $prodSize->title_ar) {{$prodSize->title_ar}} @endif</a></li>
                                @endforeach
							</ul>
						</div>
					</div>
						...
						<div class="tt-collapse-content">
							<ul class="tt-options-swatch options-middle">
                               @foreach($prodColors as $prodColor)
                                @if($prodColor->image)
                                <li @if(!empty(Cookie::get('filter_by_color')) && Cookie::get('filter_by_color')==$prodColor->id) class="active" @endif><a href="javascript:;" class="options-color tt-border filter_by_color" id="{{$prodColor->id}}"><img src="{{url('uploads/color/thumb/'.$prodColor->image)}}"  width="40" height="40"/></a></li>
                                @else
								<li @if(!empty(Cookie::get('filter_by_color')) && Cookie::get('filter_by_color')==$prodColor->id) class="active" @endif><a href="javascript:;" class="options-color tt-border filter_by_color" id="{{$prodColor->id}}" style="background-color:{{$prodColor->color_code}};"></a></li>
                                @endif
                                @endforeach
							</ul>
						</div>
					...
					<div class="tt-collapse open">
						...
						<div class="tt-collapse-content">
							<ul class="tt-list-inline">
								...
								<li><a href="javascript:;" class="filter_by_tags" id="{{$cattag}}">{{$cattag}}</a></li>
                                @endforeach
							</ul>
						</div>
					</div>
